...
* **自动化与智能化：**
    ...
    * **性能优化：** 智能同步时避免不必要的重复计算，只在有新内容时更新统计信息。
    * **增强稳定性：** 改进权限错误处理，优雅处理文件锁定和访问权限问题。
...
# 执行智能同步 (推荐)
python main.py
# 说明：只同步新帖子，有新帖子时更新活动总结，性能优化

# 强制执行全量同步
python main.py --full-sync
# 说明：重新同步所有帖子，总是生成活动总结，适合首次使用或重置
```
...
### 更新时机
- **全量同步时**：总是重新生成活动总结和热力图
- **智能同步时**：仅在检测到新帖子时才更新，避免不必要的重复计算
- **首次运行时**：自动生成初始活动总结报告
...
- 只更新已编辑的帖子
- 保持现有备份文件不变
- **智能更新活动总结**：仅在检测到新帖子时才更新热力图和活动总结，避免不必要的重复计算
